...
## ❓ 퀀트 플랫폼에서 '실시간 데이터'를 왜 서빙하나요?

사용자께서 지적하신 대로, 퀀트 투자는 초단타 매매(HFT)처럼 '급박하게' 매매하는 것이 주 목적은 아닙니다. 주로 과거 데이터에 기반해 특정 시점에 포트폴리오를 교체(리밸런싱)하는 방식입니다. 

그럼에도 불구하고 **자산운용 플랫폼 엔지니어**가 실시간 데이터를 서빙해야 하는 이유는 '매매 빈도'가 아니라 **'운용의 정확성과 리스크 관리'**에 있습니다.

### 1. 오늘의 최종 시그널 확정 (Daily Signal Confirmation)
*   **상황**: 매일 장 종료 직전이나 특정 시점에 포트폴리오를 교체해야 한다면, 그 시점의 **실시간 주가**가 반영되어야 정확한 매수/매도 수량이 결정됩니다.
*   **엔지니어링**: 하루에 한 번만 매매하더라도, 그 한 번의 의사결정을 위해 가장 최신의 데이터를 API로 즉시 공급해야 합니다.

### 2. 실시간 포지션 및 손익 모니터링 (Live PnL & Risk)
*   **상황**: 매매는 한 달에 한 번 하더라도, **내 자산의 가치는 초 단위로 변합니다.** 펀드매니저는 장 중에 자산 가치가 급변하거나 설정된 리스크 한도를 넘지 않는지 실시간으로 확인해야 합니다.
*   **엔지니어링**: 실시간 시세를 받아 현재 보유 종목과 매칭하여 초단위로 PnL을 산출하고 서빙합니다.

### 3. 주문 집행의 품질 (Execution Quality)
*   **상황**: 1,000억 원의 자산을 교체할 때, 한꺼번에 주문을 내면 가격이 튀어버립니다. 이를 방지하기 위해 장 중에 호가 상황을 보며 분할 매수하는 '알고리즘 주문'이 필요하며, 이때 실시간 데이터가 쓰입니다.
*   **엔지니어링**: 트레이더처럼 급하게 매매하기 위해서가 아니라, **'가장 싸게 잘 사기 위해'** 실시간 호가 데이터를 서빙합니다.

### 4. 킬스위치 및 이상 탐지 (Kill-switch)
*   **상황**: 퀀트 모델이 예상치 못한 시장 상황(예: 급락장)에서 오작동할 수 있습니다. 이때 실시간으로 상황을 감지하여 주문을 즉시 멈추는 인프라는 자산운용사의 필수 안전장치입니다.
