...
### 2. Parquet 포맷과의 최상의 궁합
- **Parquet(파케이)**는 S3에서 지원하고 권장하는 대표적인 **컬럼 기반(Columnar)** 저장 방식입니다.
- **효율적인 조회**: 특정 컬럼(예: 종가)만 읽어올 수 있어 전체 파일을 다 읽을 필요가 없습니다. (I/O 절감)
- **강력한 압축**: 데이터 크기를 획기적으로 줄여 S3 저장 비용과 데이터 전송 비용을 낮춥니다.
- **Athena 연동**: S3에 저장된 Parquet 파일은 AWS Athena를 통해 DB 서버 없이도 즉시 SQL 쿼리가 가능합니다.

### 3. 컴퓨팅과 저장소의 분리 (Decoupling)
