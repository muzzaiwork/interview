...
## 🤖 대표적인 퀀트 투자 모델 (Quant Models)

퀀트 투자는 수학적, 통계적 모델을 기반으로 투자 결정을 내리는 방식입니다. 대표적인 모델들은 다음과 같습니다.

### 1. 팩터 모델 (Factor Investing)
- **개념**: 주식의 수익률을 결정하는 특정 **요소(Factor)**들을 찾아내어 투자하는 방식입니다.
- **주요 팩터**:
    - **가치 (Value)**: 자산 대비 싼 주식을 삼 (**PBR, PER** 활용). *본 프로젝트의 핵심 전략*
    - **모멘텀 (Momentum)**: 최근에 오른 주식이 더 오를 것이라 믿고 투자.
    - **소형주 (Size)**: 대형주보다 소형주가 수익률이 높다는 현상에 투자. *본 프로젝트의 핵심 전략*
    - **퀄리티 (Quality)**: 우량한 재무 구조를 가진 기업에 투자 (**ROE** 등 활용).

### 2. 통계적 차익거래 (Statistical Arbitrage)
- **개념**: 서로 연관성 있는 두 종목의 가격 차이가 일시적으로 벌어졌을 때, 다시 좁혀질 것을 기대하고 투자하는 방식입니다.
- **대표 사례: 페어 트레이딩 (Pair Trading)**:
    - 예: '삼성전자'와 'SK하이닉스'는 비슷하게 움직이는데, 갑자기 하나만 급락했다면? → 싼 것을 사고 비싼 것을 팔아 차익을 챙김.

### 3. 리스크 패리티 (Risk Parity)
- **개념**: 자산의 비중이 아니라, 각 자산이 가진 **위험(변동성)의 비중**을 똑같이 맞추는 모델입니다.
- **예시**: 주식은 위험하고 채권은 안전하므로, 채권 비중을 훨씬 높여 전체 포트폴리오의 위험 수준을 균형 있게 조절합니다. (브리지워터의 **올웨더 포트폴리오**가 대표적)

### 4. 추세 추종 (Trend Following)
- **개념**: 시장의 방향성이 결정되면 그 방향으로 계속 갈 것이라고 가정하는 모델입니다.
- **활용**: 이동평균선 등을 활용하여 "오르는 말에 올라타고, 내리는 말에서 내리는" 전략입니다.

```mermaid
graph LR
    subgraph "퀀트 전략 분류"
        A[팩터 투자] --- A1[가치/모멘텀/소형주]
        B[차익거래] --- B1[가격 괴리 활용]
        C[자산배분] --- C1[리스크 패리티]
    end
    
    A1 --> Project[소형주 + 저PBR 전략]
    style Project fill:#fff3e0,stroke:#ff9800,stroke-width:2px
```
