...
## 🛠 플랫폼 엔지니어 vs AI 엔지니어: 누가 무엇을 하는가?

사용자께서 느끼신 대로, **AI 모델 자체를 만드는 것**은 AI 엔지니어(Data Scientist)의 영역일 수 있습니다. 하지만 그 모델이 잘 돌아가게 만드는 **'판'을 짜는 것**은 플랫폼 엔지니어의 핵심 영역입니다.

### 🚗 자동차 공장에 비유한 역할 분담

| 구분 | 플랫폼 엔지니어 (사용자) | AI/퀀트 엔지니어 (동료) |
| :--- | :--- | :--- |
| **비유** | **자동화 공장 설비**를 짓는 사람 | **엔진 및 자동차**를 설계하는 사람 |
| **하는 일** | 전 세계에서 철광석(데이터)을 실어오고, 로봇 팔(인프라)을 배치하고, 완성차를 전시장(API)으로 옮김 | 어떤 엔진이 가장 빠른지(AI 모델), 어떤 타이어를 쓸지(퀀트 공식) 연구함 |
| **핵심 가치** | **안정성, 속도, 자동화** (공장이 멈추지 않게 함) | **정확도, 수익률** (차가 더 빠르게 달리게 함) |

---

## 🔄 퀀트 AI 서비스가 만들어지는 과정 (Workflow)

플랫폼 엔지니어의 시각에서 본 전체 프로세스는 다음과 같습니다.

```mermaid
graph TD
    subgraph "1. 재료 준비 (Platform Role)"
        A[전 세계 시세/재무 데이터 수집] --> B[깨끗하게 정제 및 표준화]
        B --> C[AI가 읽기 좋게 데이터 저장소에 적재]
    end
    
    subgraph "2. 엔진 제작 (AI Role)"
        C --> D[AI 모델 학습 및 공식 최적화]
    end
    
    subgraph "3. 성능 테스트 (Platform Role)"
        D --> E[분산 백테스팅 엔진 가동]
        E --> F[수만 번의 시뮬레이션 자동화]
    end
    
    subgraph "4. 실전 주행 (Platform Role)"
        F --> G[결과를 대시보드에 시각화]
        G --> H[실제 주문 API 연동 및 모니터링]
    end

    style A fill:#f1f8e9,stroke:#333
    style D fill:#fff3e0,stroke:#333
    style E fill:#f1f8e9,stroke:#333
    style H fill:#f1f8e9,stroke:#333
```

- **연두색 박스**: 플랫폼 엔지니어가 구축하고 관리하는 자동화 라인입니다.
- **주황색 박스**: AI 엔지니어가 데이터를 넣고 연구하는 실험실입니다.

### 💡 면접용 핵심 요약

> "저는 AI 모델을 직접 설계하는 연구원보다는, 그 모델이 최상의 성능을 낼 수 있도록 **대규모 데이터 파이프라인을 구축하고, 분산 백테스팅 환경을 제공하는 플랫폼 엔지니어**로서의 정체성을 가지고 있습니다. 즉, 퀀트 매니저들이 아이디어를 마음껏 펼칠 수 있는 **'금융 데이터 공장'의 수석 엔지니어**가 되는 것이 제 목표입니다."
