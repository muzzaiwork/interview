...
- **💡 Apache Parquet이란? (Columnar Storage)**: 
    - **개념**: CSV와 같은 '행(Row)' 단위가 아닌 **'열(Column)' 단위**로 데이터를 저장하는 고성능 파일 포맷입니다.
    - **왜 퀀트에서 쓰나요?**: 
        - **속도**: 수천 개의 팩터 중 'PBR' 하나만 분석하고 싶을 때, CSV는 전체 파일을 다 읽어야 하지만 Parquet은 PBR 열만 골라서 읽으므로 I/O가 압도적으로 빠릅니다.
        - **압축**: 데이터 타입별로 최적화된 압축을 지원하여 저장 공간을 CSV 대비 최대 90%까지 절약합니다.
        - **스키마 보존**: 데이터 타입(Float, Int 등)이 파일 내에 명시되어 있어, 데이터를 읽을 때 별도의 타입 변환 과정이 필요 없습니다.
